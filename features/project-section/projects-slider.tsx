...
export function ProjectSlider(){
    ...
    return (<div className="px-8 md:px-0 h-full w-screen md:w-4/5"><div className="relative w-full mt-4 md:mt-10 h-4/5 md:h-full">
            ...
            return (<div 
                ...
                className={`h-7/8 md:h-3/4 bg-white border-1 border-primary absolute rounded-sm ${index === 0 ? "animate-scale-in" : ""}`}>
                    <div className="flex flex-col-reverse md:flex-row gap-0 md:gap-4 h-full">
                        ...
                        <div className="grow h-20 md:max-h-full rounded-r-sm overflow-hidden"><Image alt="Project Screenshot" src={project.imageUrl} width={770} height={790} className="object-cover max-h-full rounded-r-sm"/></div>
                    </div>
        </div>);})}
    </div>
    <div>
        <div className="flex justify-center gap-20 mt-6 md:mt-0"><Button disabled={viewedProjects.length==0} className="cursor-pointer w-10 h-10 flex items-center justify-center rounded-full font-bold bg-white text-primary border-2 border-primary" onClick={viewPrevious}>&larr;</Button><Button disabled={unviewedProjects.length==1} className="cursor-pointer w-10 h-10 flex items-center justify-center rounded-full font-bold" onClick={viewNext}>&rarr;</Button></div>
    </div>
    </div>);
}