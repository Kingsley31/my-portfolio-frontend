...
export function ProjectSlider(){
    ...
    if (!mounted) return (<div className="flex items-center md:items-start justify-center">
                              <Skeleton className="w-2/3 h-70 md:w-90 md:h-90 rounded-sm" />
                            </div>);
    return (<div className="px-4 md:px-0 h-full w-screen md:w-4/5"><div className="relative w-full mt-4 md:mt-10 h-full md:h-full">
        {unviewedProjects.map((project,index) => {
            ...
            return (<div 
                ...
                className={`h-8/10 md:h-3/4 bg-white border-1 border-primary absolute rounded-sm ${index === 0 ? "animate-scale-in" : ""}`}>
                    <div className="flex flex-col-reverse md:flex-row gap-4 h-full">
                        <div className="flex flex-col gap-2 justify-between h-full md:h-auto">
                            ...
                            <div className="mb-8 md:mb-6 mt-6 md:mt-0"><span className="inline-block animate-cta-pulse cursor-pointer rounded-l-0 rounded-r-md bg-primary font-bold text-white py-2 px-4 text-xs">View Details &gt;&gt;</span></div>
                        </div>
                        <div className="grow max-h-2/5 md:max-h-full rounded-r-sm overflow-hidden"><Image alt="Project Screenshot" src={project.imageUrl} width={770} height={790} className="object-cover max-h-full rounded-r-sm"/></div>
                    </div>
        </div>);})}
    </div>
    <div>
        <div className="flex justify-center gap-20"><Button disabled={viewedProjects.length==0} className="cursor-pointer w-10 h-10 flex items-center justify-center rounded-full font-bold bg-white text-primary border-2 border-primary" onClick={viewPrevious}>&larr;</Button><Button disabled={unviewedProjects.length==1} className="cursor-pointer w-10 h-10 flex items-center justify-center rounded-full font-bold" onClick={viewNext}>&rarr;</Button></div>
    </div>
    </div>);
}